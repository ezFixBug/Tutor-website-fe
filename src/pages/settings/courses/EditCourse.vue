<template>
  ...
  <div class="container-fluid">
    ...
    <div class="dashboard-heading mb-5">
      <h1 class="fs-22 font-weight-semi-bold">Chỉnh sửa khóa học</h1>
    </div>
    <form
                  ...
                  <div class="col-4">
                    <label class="ui-checkbox ui-checkbox-primary">
                      <input
                        ...
                        v-model="is_free"
                      />
                      <span class="input-span"></span>Khóa học miễn phí
                    </label>
                  </div>
                </div>
              </div>
  ...
  <div class="course-submit-btn-box pb-4">
    <router-link
      ...
      >Trở về</router-link
    >
    <button class="btn theme-btn" type="button" @click="handleEditCourse">
      Chỉnh sửa khóa học
    </button>
...
export default {
  async created() {
    ...
    if (get(res, "data.result", false)) {
      ...
      for (const day in this.course.schedule) {
        if (schedule.hasOwnProperty(day)) {
          schedule[day] = this.course.schedule[day];
        }
      }
      this.course.schedule = schedule;

      const classes = this.course.classes.map((item) => {
        return item.id;
      });

      this.course.classes = classes;
      const subjects = this.course.subjects.map((item) => {
        return item.id;
      });
      this.course.subjects = subjects;

      this.is_free = this.course.price ? false : true;
    }
    this.listOptionProvinces = await $http.getProvinces();
    this.listOptionDistricts = await $http.getDistricts(
      this.course.province_id
    );
  ...
  methods: {
    ...
    async handleEditCourse() {
      ...
      const res = await $http.post("/edit-course/", params);
      this.is_loading = false;
      if (get(res, "data.result", false)) {
        createToast("Chỉnh sửa khóa học thành công!", {
          type: "success",
          timeout: 6000,
        });
